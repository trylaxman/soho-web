      ...
      <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="relative z-10 w-full max-w-5xl">
          <div className="mb-14 text-center">
            <Link href="/" className="mx-auto mb-8 inline-flex items-center gap-3">
              <span className="flex h-14 w-14 items-center justify-center rounded-full border border-[#8f6b2f] bg-[#0f0f0f] text-xl font-semibold text-[#d7b36a]">
                SC
              </span>
              <span className="text-left">
                <span className="block font-serif text-2xl tracking-wide text-[#e3bd74]">SoHo Cleaning Group</span>
                <span className="block text-[10px] uppercase tracking-[0.28em] text-[#a98a51]">Pristine Spaces. Premium Care.</span>
              </span>
            </Link>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {roles.map((role) => {
              ...
              return (
                <label
                  key={role.id}
                  className={`group relative cursor-pointer rounded-[32px] border p-4 transition duration-300 sm:p-5 ${
                    isSelected
                ...
                >
                  ...
                  <span
                    className={`absolute right-6 top-6 flex h-7 w-7 items-center justify-center rounded-full border transition ${
                      isSelected ? 'border-[#d6ab5f] bg-[#d6ab5f]' : 'border-[#6f6149] bg-transparent'
                    }`}
                  >
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-5">
            <Link
              ...
              className={`w-full max-w-sm rounded-2xl px-6 py-4 text-center text-base font-semibold transition sm:w-auto sm:min-w-64 ${
                selectedRole
            ...
            <p className="text-sm text-[#b8ad9a]">
              Already have an account?{' '}
              <Link href="/login" className="font-medium text-[#e3bd74] underline-offset-4 hover:underline">
                Log in
              </Link>
            </p>
          </div>
